...
function countProfit(shoppers) {
  ...
  for (let i in shoppers) {
    let find = listBarang.find((a) => a[0] === shoppers[i].product);

    if (find) {
      if (find[2] >= shoppers[i].amount) {
        find[2] -= shoppers[i].amount;
        listShopper.push([find[0], shoppers[i].name, shoppers[i].amount]);
      }
    }
  }
  for (let i in listBarang) {
    let filter = listShopper.filter((a) => a[0] === listBarang[i][0]);
    if (!filter) {
      result.push({
      ...
      for(let i in filter){
        totalMount += filter[i][2]
        shopper.push(filter[i][1])
      }
      ...
      result.push({
        ...
        shoppers: shopper,
        leftOver: listBarang[i][2],
        totalProfit: totalMount * listBarang[i][1],
      });
    }
  } 
  return result
